> **[AI-ONLY]** 功能 PRD 模板（Feature PRD）
> **[AI-ONLY]** **使用场景**：一个完整功能模块，面向设计、研发、测试多个受众。
> **[AI-ONLY]** **复制方法**：运行 `/new-prd feature [标题]`，AI 自动从此模板创建。
...
> **[AI-ONLY]** 从需求卡片（rdd.md）中摘取关键结论，为阅读 PRD 的人提供最少必要背景，不重复需求分析过程。
...
> **[AI-ONLY]** **编写规则**：只写本 PRD 新引入的业务术语；产品已有术语统一参见 `context/business-glossary.md`，不在此重复定义。
...
> **[AI-ONLY]** **编写规则**：只展示本 PRD 在整体产品结构中新增/调整的节点；完整产品功能树见 `context/product-feature-map.md`。
...
> **[AI-ONLY]** **编写规则**：
> - 只写跨功能的全局约束（影响 2 个及以上功能的规则）；单功能内部异常留在对应 §8.x.3
> - 编号格式：BR-XX（如 BR-01），PRD 内唯一，每个 PRD 从 BR-01 开始
> - 无跨功能规则时保留本节并注明「本需求无跨功能业务规则」
> - 示例：产品线行为差异、多层优先级模型、术语语义定义、合规基线等

| 规则编号 | 规则描述 | 备注 |
| --- | --- | --- |
| BR-01 | （示例）国际站采用快照方案，配置变更仅对后续新流程生效；国内站采用实时方案，配置变更立即对所有运行中流程生效 | 两条产品线设计差异，非 Bug |
| BR-02 | （示例）流程级配置 > 租户级配置，流程级"禁止下载"时无论租户配置如何均禁止 | 两层优先级模型 |
...
> **[AI-ONLY]** 权限模型参考：`context/permission-model.md`（使用前需初始化）。
...
#### 8.10 对外 OpenAPI 变更说明（仅记录面向外部开发者的 API 新增/修改/废弃，内部服务调用不填此节；不涉及则填「不涉及」）

| 变更类型 | 接口路径 | HTTP 方法 | 变更内容描述 | 是否影响对外文档 |
| --- | --- | --- | --- | --- |
| 新增/修改/废弃 | /v1/xxx | GET/POST/PUT/DELETE | 新增参数 X，类型 string，必填，说明... | 是/否 |

> **[AI-ONLY]** 飞轮触发：本节有实质内容（非「不涉及」、非空）**且**"是否影响对外文档"列至少有一行填"是"，且 PRD 在正式区时，`/update-prd` 完成后自动扫描此节并提议执行 `/update-openapi`；填「不涉及」或所有行均填"否"时静默跳过。内部接口调整不得填入此节，不触发飞轮。
...
> **[AI-ONLY]** 端清单参考：`context/platform-support.md`（使用前需初始化）。
...
> **[AI-ONLY]** **编写规则**：
> - 本章节是 §6 Gherkin 验收标准的汇总索引视图，不独立编写
> - AI 在所有章节（§6-§9）填充完成后，从 §6 中每条 Gherkin AC 提炼一句话描述，统一回填本章节
> - 格式：`- [ ] **AC-N**：一句话描述`，编号从 AC-1 开始连续递增
> - §6 未填充时保留本节并注明「待 §6 验收标准完善后自动生成」

- [ ] **AC-1**：（示例）租户关闭下载开关后，签署中合同页面不展示下载按钮，后端拦截返回 403
- [ ] **AC-2**：（示例）发起时配置"带已签署印章"，签署中对方下载文件包含当前已签署印章
- [ ] **AC-3**：（示例）流程完成后，无论格式配置如何，对方可正常下载合同文件
