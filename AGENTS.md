...
| 元数据 | 值 |
|--------|-----|
| 基于 template 版本 | v6.1（2026-05-21） |
| 基于 template 提交 | `9717a8e` |
| 同步日期 | 2026-05-21 |
| 同步内容摘要 | F-015 截图库导航双层架构 + CTX-FLY-005/006；ID 冲突修复（远程 registry 同步 + 全量替换）；F-048 OpenAPI 规范质检（11维度37规则 + 三命令差异化质检门控） |
...
| 更新/新建 PRD 移入正式区，§8 含页面/弹窗描述 | 扫描 §8 未录入导航图的新页面 → 输出末尾追加补录建议（CTX-FLY-005） |
| /generate-prototype 完成 | 对比原型页面列表与导航图节点 → 有缺口时提示补录（CTX-FLY-006） |
