...
| PRD问答 / 需求答疑 / 这个功能怎么... / 我想问产品问题 / PRD里... / 这个功能的规则/权限/字段/边界 | `.claude/commands/prd-qa.md` |
...
2. **Phase 1（轻量）**：基于 3 个背景文件快速生成用户故事草稿，等用户确认方向后阻塞
   - Phase 1 含商业层升级检测：若发现"目标用户不明确""商业价值存疑"等信号，建议先用 `jobs-to-be-done` 技能做客户研究（不强制，等用户选择）
3. **Phase 2（深度）**：多轮对话，对话信号驱动按需加载更多 context，生成完整 RDD 卡片
   - Phase 2 含商业层升级检测：每轮澄清结束后判断一次，命中时最多提示 1 次，不重复打断
...
### prd-qa：自然语言触发 → 问题收敛 → 四层检索 → 强制来源标注

`prd-qa` 执行前先完成入口行为和问题收敛，再进入检索：

1. **入口行为（KQA-RTG-001）**：若用户表达"想问产品问题"但未说出具体问题 → 输出引导语「我会基于产品上下文和 PRD 帮你检索，请说出你的问题。」→ 等待具体问题，**不提前检索**
2. **清晰度收敛（KQA-QRY-004）**：问题无锚点（无功能名/PRD ID/字段名）时 → 一句话追问，最多 3 轮；3 轮后仍无锚点 → 标注「基于有限信息检索，结果可能不精确」后进入检索
3. **多意图处理（KQA-QRY-005）**：问题含 2+ 独立意图 → 拆分有序列表逐一回答；单次最多 3 个，超出提示分次提问
4. **来源标注强制（KQA-ANS-002）**：有文档命中 → 必须输出来源标注；无命中 → 走兜底声明，不推断、不生成来源
5. **追问引导（KQA-GDE-001）**：答案末尾追加「**你可能还想问：**」2-3 条完整问句，范围限同一 PRD；兜底回复时不触发

**路由边界**：含"代码"/"报错"/"这段代码"/"怎么实现"/"技术方案"/"竞品"/"市场调研"等关键词 → 提示超出知识库范围；含"帮我改"/"优化一下"/"新增"等改进意图 → 走需求澄清路径

**Codex 等效触发示例**：
- "我想问几个产品问题" → 输出引导语，等待具体问题
- "F-007 知识库提问的权限规则是怎么说的" → 有锚点，直接进入检索
- "这个功能的字段清单是什么" → 无锚点，追问功能名
